...
const ReadOnlyTextField: React.FC<ReadOnlyTextFieldProps> = ({ label, value }) => (
    <TextField
        label={label}
        value={value}
        InputProps={{
            ...
            <Paper elevation={3} sx={{ padding: 2, margin: '20px 0' }}>
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} md={6}>
                        <ReadOnlyTextField label="Street Number" value={user.billing_street_num} />
                    </Grid>
                    <Grid item xs={12} md={6}>
                        <ReadOnlyTextField label="Street Name" value={user.billing_street_name} />
                    </Grid>
                    <Grid item xs={12} md={6}>
                        <ReadOnlyTextField label="Unit Number" value={user.billing_unit_number} />
                    </Grid>
                    <Grid item xs={12} md={6}>
            ...
            <Paper elevation={3} sx={{ padding: 2, margin: '20px 0' }}>
                <Grid container spacing={2}>
                    {user.locations.map((location, index) => (
                        <Grid item xs={12} sm={6} md={4} key={index}>
                            <Paper elevation={2} sx={{ padding: 1, margin: '8px 0' }}>
                                <ReadOnlyTextField label="Location Name" value={location.name} />
